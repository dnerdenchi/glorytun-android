...
#ifdef __ANDROID__
extern int gt_socket(int domain, int type, int protocol);
...
#ifdef __ANDROID__
    int fd;  /* パス専用UDPソケット。-1 = mud の primary_fd を使用 */
#endif
...
#ifdef __ANDROID__
/* パスに専用ソケットを紐付ける (Android マルチパス用)
 * fd は呼び出し元が socket() + VpnService.protect() + Network.bindSocket() 済みであること。
 * mud が内部で SO_REUSEADDR / IP_PKTINFO を設定し、local アドレスにバインドして epoll に登録する。 */
int mud_set_path_socket(struct mud *mud, union mud_sockaddr *local, int fd);

/* パスの専用ソケットを解除して close する */
int mud_clear_path_socket(struct mud *mud, union mud_sockaddr *local);
#endif
